...
def TrainNN(num_hiddens, eps, momentum, num_epochs):
  ...
  for epoch in xrange(num_epochs):
    ...
    #misprediction = 1-prediction
    #print misprediction
    #eva = prediction - misprediction
    #print eva
    ...
    #print MCE	
    #break
    '''if (prediction <(1-prediction)  and target_train == 0):
    ...
    sys.stdout.write('\rStep %d Train CE %.5f Validation CE %.5f Mean Classification Error %0.5f' % (epoch, train_CE, valid_CE,MCE))
    sys.stdout.flush()
    if (epoch % 100 == 0):
      #ShowMeans(W1)	
      sys.stdout.write('\n')
  ...
  return W1, W2, b1, b2, train_error, valid_error, mean_classification_error
...
def DisplayErrorPlot(train_error, valid_error, mean_classification_error):
  ...
  plt.plot( range(len(mean_classification_error)),mean_classification_error, 'r' , label='Classification')
  plt.xlabel('Epochs')
...
def main():
  ...
  momentum = 0.0
  num_epochs = 5000
  W1, W2, b1, b2, train_error, valid_error,mean_classification_error = TrainNN(num_hiddens, eps, momentum, num_epochs)
